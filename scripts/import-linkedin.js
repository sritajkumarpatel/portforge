...
function parseLinkedIn(text) {
  ...
  const data = { name: "", headline: "", summary: "", skills: [], experience: [], certifications: [], education: [], awards: [], languages: [] };
  let section = null;
  ...
  for (const line of lines) {
    ...
    if (lower.startsWith("top skills")) { section = "skills"; continue; }
    if (lower.startsWith("certifications")) { section = "certifications"; continue; }
    if (lower.startsWith("honors") || lower.startsWith("awards")) { section = "awards"; continue; }
    if (lower.startsWith("languages")) { section = "languages"; continue; }
    if (lower.startsWith("education")) { section = "education"; continue; }
    if (lower.startsWith("experience") && !lower.includes("summary")) { section = "experience"; continue; }
    if (lower.startsWith("summary")) { section = "summary"; continue; }
    ...
    switch (section) {
      ...
      case "experience": {
        if (/^(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)/i.test(line) && line.includes("-")) {
      ...
      case "awards":
        if (line.length > 5 && line.length < 80) {
          data.awards.push({ title: line.replace(/^[•\-–—]\s*/, ""), year: 0, company: "", description: "" });
        }
        break;
...
function generateConfig(data) {
  return {
    ...
    titles: [data.headline || "Your Title", ...data.skills.slice(0, 4).map((s) => s.replace(/\(.*\)/, "").trim())],
    sections: [
      ...
      { id: "education", label: "Education", navLabel: "Education", enabled: data.education.length > 0 },
    ],
...
function generateExperience(data) {
  return data.experience.slice(0, 4).map((company) => ({
    ...
    duration: company.roles.length > 0
      ? (() => {
...
async function main() {
  ...
  fs.writeFileSync(path.join(OUTPUT_DIR, "src", "config.json"), JSON.stringify(generateConfig(data), null, 2));
  fs.writeFileSync(path.join(OUTPUT_DIR, "src", "data", "aboutMe.json"), JSON.stringify(generateAboutMe(data), null, 2));
  fs.writeFileSync(path.join(OUTPUT_DIR, "src", "data", "experience.json"), JSON.stringify(generateExperience(data), null, 2));
  fs.writeFileSync(path.join(OUTPUT_DIR, "src", "data", "certifications.json"), JSON.stringify(generateCertifications(data), null, 2));
  fs.writeFileSync(path.join(OUTPUT_DIR, "src", "data", "education.json"), JSON.stringify(generateEducation(data), null, 2));
  fs.writeFileSync(path.join(OUTPUT_DIR, "src", "data", "awards.json"), JSON.stringify(generateAwards(data), null, 2));

  console.error(`Generated JSON files in ${OUTPUT_DIR}/src/data/ and ${OUTPUT_DIR}/src/config.json`);
}
